...
const isFullWidth = computed(() => route.meta.layoutFullWidth || route.path.startsWith('/chat'));
const isFullscreen = computed(() => route.meta.layout === 'fullscreen' || route.path.startsWith('/chat'));
const isFixedLayout = computed(() => route.meta.layoutFixed || isFullscreen.value);

const sidebarWidth = computed(() => {
    if (isFullscreen.value) return '0px';
    return navStore.isSidebarCollapsed ? 'var(--sidebar-width-collapsed)' : 'var(--sidebar-width)';
});
...
<template>
    <div class="min-h-screen bg-[var(--surface-primary)]">
        ...
        <!-- Main Content -->
        <div 
            :class="['lg:transition-[margin] lg:duration-300 flex flex-col w-full min-w-0', isFixedLayout ? 'h-[100dvh] overflow-hidden' : 'min-h-[100dvh]']" 
            :style="mainStyles"
        >
            <!-- Header -->
            ...
            <!-- Page Content -->
            <main :class="['flex-1 flex flex-col', (isFullWidth || isFullscreen) ? 'p-0' : 'p-6']">
                <!-- Breadcrumbs -->
                <Breadcrumbs 
                    v-if="!isFullscreen && (!isFullWidth || route.meta.showBreadcrumbs)" 
                    class="mb-6" 
                ...
                <RouterView v-slot="{ Component, route: currentRoute }">
                    <Transition 
                        ...
                        mode="out-in"
                    >
                        <component :is="Component" :key="currentRoute.path" class="flex-1 w-full min-w-0" />
                    </Transition>
                    <component 
                        v-else 
                        :is="Component" 
             ...
             <!-- App Footer -->
            <footer v-if="!isFullscreen && !isFixedLayout" class=" py-6 px-6 border-t border-[var(--border-default)] bg-[var(--surface-primary)]">
                <div class="flex flex-col md:flex-row items-center justify-between gap-4 text-xs text-[var(--text-tertiary)]">
                    <div class="flex items-center gap-2">
                        <span class="font-medium text-[var(--text-secondary)]">CoreSync v2.0.0</span>
                    ...
                    <div class="flex items-center gap-6">
                        ...
                        <a href="/terms" class="hover:text-[var(--text-primary)] transition-colors">Terms of Service</a>
                         <a href="/public/faq" class="hover:text-[var(--text-primary)] transition-colors">Help Center</a>
                    </div>
                </div>
            </footer>
        </div>
        ...
        <AccountBlockedModal
            v-if="authStore.showBlockedModal && authStore.statusChangeEvent?.status"
