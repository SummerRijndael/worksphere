...
import { Button, Avatar, Textarea, Dropdown, Modal } from "@/components/ui";
import {
    Calendar,
    Clock,
    User,
...
const getStatusValue = (task: any) =>
    task?.status?.value || task?.status || "open";

// Member options for assign dropdown
const memberOptions = computed(() => {
    const members =
        props.projectMembers?.length > 0
...
const fetchTaskDetails = async () => {
    ...
    try {
        ...
        const response = await axios.get(
            `/api/teams/${props.teamId}/projects/${props.projectId}/tasks/${taskId}`,
        );
...
const fetchMembers = async () => {
    if (props.projectMembers?.length > 0 || !props.teamId || !props.projectId)
        return;
    try {
        const response = await axios.get(
            `/api/teams/${props.teamId}/projects/${props.projectId}`,
        );
...
const fetchComments = async () => {
    ...
    try {
        ...
        const response = await axios.get(
            `/api/teams/${props.teamId}/projects/${props.projectId}/tasks/${taskId}/comments`,
        );
...
const fetchStatusHistory = async () => {
    ...
    try {
        const taskId = localTask.value.public_id || localTask.value.id;
        const response = await axios.get(
            `/api/teams/${props.teamId}/projects/${props.projectId}/tasks/${taskId}/status-history`,
        );
...
const assignTask = async (assigneeId: string) => {
    ...
    try {
        ...
        const response = await axios.post(
            `/api/teams/${props.teamId}/projects/${props.projectId}/tasks/${taskId}/assign`,
            {
...
const performArchive = async () => {
    try {
        ...
        const taskId = localTask.value.public_id || localTask.value.id;
        await axios.post(
            `/api/teams/${props.teamId}/projects/${props.projectId}/tasks/${taskId}/archive`,
        );
        toast.success("Task archived");

        // Refresh task to show archived status
        const response = await axios.get(
            `/api/teams/${props.teamId}/projects/${props.projectId}/tasks/${taskId}`,
        );
        localTask.value = response.data.data || response.data;
...
<template>
    <DialogRoot v-model:open="isOpen">
        <DialogPortal>
            <DialogOverlay
                class="fixed inset-0 z-[1040] bg-black/60 backdrop-blur-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0"
            />
            <DialogContent
                class="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-[1050] w-full max-w-4xl max-h-[92vh] bg-[var(--surface-primary)] rounded-xl shadow-2xl overflow-hidden focus:outline-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 duration-200"
            >
                <div v-if="localTask" class="flex flex-col h-full max-h-[92vh]">
                    <!-- Compact Header -->
                    <div
                        class="flex items-center justify-between px-6 py-4 border-b border-[var(--border-subtle)] bg-[var(--surface-primary)]"
                    >
                        <div class="flex items-center gap-3 min-w-0">
                            <span
                                class="text-xs font-mono text-[var(--text-muted)] bg-[var(--surface-secondary)] px-2 py-1 rounded"
                            >
                                #{{
                                    (
                                        localTask.public_id || localTask.id
                                    )?.substring(0, 8)
                                }}
                            </span>
                            <span
                                :class="[
                                    getStatus(getStatusValue(localTask)).bg,
                                    getStatus(getStatusValue(localTask)).color,
                            ...
                            <span
                                :class="[
                                    getPriority(localTask.priority).bg,
                                    getPriority(localTask.priority).color,
                                    'px-2.5 py-1 rounded-full text-xs font-medium',
                                ]"
                            >
                    ...
                    <div class="flex flex-1 overflow-hidden">
                        ...
                        <div class="flex-1 overflow-y-auto">
                            <div class="p-6 space-y-6">
                                ...
                                <!-- Deprecation Warning -->
                                <div
                                    class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-3 mb-4 flex items-center gap-3"
                                >
                                    <AlertTriangle
                                ...
                                <div>
                                    <DialogTitle
                                        class="text-xl font-semibold text-[var(--text-primary)] mb-3"
                                    >
                                        (DEPRECATED) {{ localTask.title }}
                                    </DialogTitle>
                                    <DialogDescription class="sr-only"
                                        >Task details and
                                        management</DialogDescription
                                    >
                                    <div
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    ...
                                    <div class="relative">
                                        <label
                                            class="text-xs font-medium text-[var(--text-muted)] uppercase tracking-wide mb-2 block"
                                            >Assignee</label
                                        >
                                        <button
                                            @click="
                                        ...
                                        >
                                            ...
                                            <div
                                                v-else
                                                class="w-8 h-8 rounded-full bg-[var(--surface-tertiary)] flex items-center justify-center border-2 border-dashed border-[var(--border-default)]"
                                            >
                                                <UserPlus
                                            ...
                                            <ChevronRight
                                                class="w-4 h-4 text-[var(--text-muted)] ml-auto"
                                            />
                                        </button>
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                <div
                                                    class="h-px bg-[var(--border-subtle)] my-1"
                                                ></div>
                                                <button
                                                    v-for="member in memberOptions"
                                                    :key="member.value"
                                                    @click="
                                                        assignTask(member.value)
                                                    "
                                                    class="w-full px-3 py-2.5 text-left text-sm hover:bg-[var(--surface-secondary)] flex items-center gap-3 transition-colors"
                                                    :class="{
                                                ...
                                                >
                                                    ...
                                                    <CheckCircle2
                                                        v-if="
                                                            ...
                                                            member.value
                                                        "
                                                        class="w-4 h-4 text-[var(--interactive-primary)] ml-auto"
                                                    />
                                                </button>
                                            </div>
                                        </Transition>
                                    ...
                                    <div>
                                        ...
                                        >
                                        <div
                                            class="flex items-center gap-3 p-3 rounded-lg border border-[var(--border-default)] bg-[var(--surface-secondary)]/30"
                                        >
                                            <div
                                                class="w-8 h-8 rounded-full bg-[var(--surface-tertiary)] flex items-center justify-center"
                                            >
                                    ...
                                    <div>
                                        ...
                                        >
                                            ...
                                            >
                                                {{
                                                    localTask.estimated_hours
                                                        ? `${localTask.estimated_hours} hours`
                                                        : "Not estimated"
                                                }}
                                            </span>
                                        </div>
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <span
                                                ...
                                                class="bg-[var(--surface-tertiary)] text-[var(--text-muted)] text-xs px-1.5 py-0.5 rounded-full"
                                            >
                                                {{ comments.length }}
                                            </span>
                                        </button>
                                        ...
                                        <div class="space-y-3 mt-4">
                                            ...
                                            >
                                                ...
                                                <div class="flex-1 min-w-0">
                                                    <div
                                                        class="bg-[var(--surface-secondary)] rounded-lg px-4 py-3"
                                                    >
                                                        <div
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <div class="flex-1 pb-2">
                                                ...
                                                <div
                                                    v-if="entry.notes"
                                                    class="mt-2 text-sm text-[var(--text-secondary)] bg-[var(--surface-secondary)] p-2 rounded border border-[var(--border-subtle)]"
                                                >
                                                    {{ entry.notes }}
                                                </div>
                                                <p
                        ...
                        <div
                            class="w-64 border-l border-[var(--border-subtle)] bg-[var(--surface-secondary)]/30 overflow-y-auto shrink-0"
                        >
                            <div class="p-4 space-y-6">
                                ...
                                <div>
                                    ...
                                    <div class="space-y-1.5">
                                        ...
                                        <button
                                            class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-[var(--text-secondary)] hover:bg-[var(--surface-tertiary)] transition-all text-left opacity-50 cursor-not-allowed"
                                            disabled
                                        >
                                            <Paperclip
                                                class="w-4 h-4 text-[var(--text-muted)]"
                                ...
                                <!-- Task Meta -->
                                <div
                                    class="pt-4 border-t border-[var(--border-subtle)]"
                                >
                                    <div
                                        class="text-xs text-[var(--text-muted)] space-y-1.5"
                                    >
                                        <p v-if="localTask.created_at">
                                            Created
                                            {{ timeAgo(localTask.created_at) }}
                                        </p>
                                        <p v-if="localTask.updated_at">
                                            Updated
                                            {{ timeAgo(localTask.updated_at) }}
                                        </p>
                                    </div>
                                </div>
                            </div>
                ...
                >
                    ...
                    <DialogDescription class="sr-only"
                        >Loading task details</DialogDescription
                    >
                    <div
                        class="w-8 h-8 border-2 border-[var(--interactive-primary)] border-t-transparent rounded-full animate-spin mb-4"
                    ></div>
    ...
    >
        <template #footer>
            <Button variant="ghost" @click="deleteModalOpen = false"
                >Cancel</Button
            >
            <Button
                variant="danger"
    ...
    <Modal
        ...
        title="Archive Task"
        description="Are you sure you want to archive this task? It will be hidden from the main view but can be restored later."
        size="sm"
    >
        <template #footer>
            <Button variant="ghost" @click="archiveModalOpen = false"
                >Cancel</Button
            >
            <Button @click="performArchive" :loading="isArchiving"
                >Archive Task</Button
            >
        </template>
    </Modal>
