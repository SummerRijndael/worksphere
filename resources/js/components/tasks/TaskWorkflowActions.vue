...
import { CheckCircle, XCircle, ArrowRight, Play, Archive, Pause, RefreshCw } from "lucide-vue-next";
...
const canSubmitForQa = computed(() => {
    return props.task.status === "in_progress" || props.task.status === "on_hold";
});
...
const availableActions = computed(() => {
    ...
    const rawStatus = typeof props.task.status === 'object' && props.task.status?.value 
        ? props.task.status.value 
        : props.task.status;

    // Operator Actions
    if (rawStatus === "open" || rawStatus === "draft") {
        actions.push({ id: "start", label: "Start Task", icon: Play, variant: "primary" });
    }
    
    if (rawStatus === "in_progress") {
         // Check if checklist is complete
         const checklist = props.task?.checklist || [];
         const checklistComplete = checklist.length === 0 || checklist.every((item: any) => item.completed || item.is_completed);
         ...
         actions.push({ 
             ...
             tooltip: !checklistComplete ? "Complete all checklist items first" : undefined
         });
         actions.push({ id: "hold", label: "Put on Hold", icon: Pause, variant: "secondary" });
    }

    if (rawStatus === "on_hold") {
        actions.push({ id: "start", label: "Resume", icon: Play, variant: "primary" });
    }

    // QA Actions
    if (rawStatus === "submitted") {
        actions.push({ id: "start_qa", label: "Start QA Review", icon: Play, variant: "primary" });
    }

    if (rawStatus === "in_qa") {
        actions.push({ id: "qa_approve", label: "Approve (To PM)", icon: CheckCircle, variant: "success" });
        actions.push({ id: "qa_reject", label: "Reject (To Operator)", icon: XCircle, variant: "danger" });
    }

    // PM Actions
    if (rawStatus === "pm_review") {
        actions.push({ id: "pm_approve", label: "Approve (To Client)", icon: CheckCircle, variant: "success" });
        actions.push({ id: "pm_reject", label: "Reject (To QA)", icon: XCircle, variant: "danger" });
    }

    // Client/Final Actions
    if (rawStatus === "sent_to_client") {
        actions.push({ id: "client_approve", label: "Client Approved", icon: CheckCircle, variant: "success" });
        actions.push({ id: "client_reject", label: "Client Rejected", icon: XCircle, variant: "danger" });
    }
    
    if (rawStatus === "client_approved") {
        actions.push({ id: "complete", label: "Mark Completed", icon: CheckCircle, variant: "primary" });
    }
    
    if (rawStatus === "rejected" || rawStatus === "client_rejected") {
        actions.push({ id: "restart", label: "Restart Task", icon: RefreshCw, variant: "primary" });
    }
...
const requiresNote = (actionId: string) => {
    return ["qa_reject", "pm_reject", "client_reject", "client_approve", "submit_qa", "pm_approve", "hold"].includes(actionId);
};
...
const isChecklistComplete = computed(() => {
    ...
    return checklist.every((item: any) => item.completed || item.is_completed);
});
...
const initiateAction = (actionId: string) => {
    ...
    if (actionId === 'submit_qa' && !isChecklistComplete.value) {
        emit('error', 'All checklist items must be completed before submitting for QA');
        return;
    }
...
<template>
    <div v-if="availableActions.length > 0" class="flex flex-wrap gap-2">
        <Button
            ...
            :key="action.id"
            :variant="action.variant === 'primary' ? 'primary' : (action.variant === 'danger' ? 'danger' : (action.variant === 'secondary' ? 'secondary' : 'outline'))"
            :class="[
                action.variant === 'success' ? 'bg-emerald-600 hover:bg-emerald-700 text-white border-emerald-600' : ''
            ]"
            size="sm"
        ...
        <Modal 
            ...
            :title="currentAction === 'hold' ? 'Put Task on Hold' : 'Add Note / Comment'"
            :description="currentAction === 'hold' ? 'Please provide a reason for putting this task on hold.' : 'Please add a note or reason for this action.'"
            size="sm"
        >
            <Textarea
                v-model="actionNote"
                :placeholder="currentAction === 'hold' ? 'Enter reason for hold...' : 'Enter details here...'"
                rows="3"
                class="w-full"
            />
            <template #footer>
                <Button variant="ghost" @click="noteModalOpen = false">Cancel</Button>
                <Button @click="confirmModalAction" :disabled="submitting || !actionNote">
