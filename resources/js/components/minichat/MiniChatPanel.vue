...
const dmChats = computed(() => chatStore.chats.filter((c) => c.type === "dm"));
...
const filteredPeople = computed(() => {
    ...
    if (peopleSearchQuery.value) {
        return chatStore.people.filter(
            (p) =>
                p.name
                    .toLowerCase()
                    .includes(peopleSearchQuery.value.toLowerCase()) ||
                p.email
                    .toLowerCase()
                    .includes(peopleSearchQuery.value.toLowerCase()),
        );
    ...
    const users = Array.from(presenceUsers.value.values())
        .filter(
            (u) =>
                u.status !== "offline" &&
                u.status !== "invisible" &&
                u.public_id !== authStore.user?.public_id,
        )
        .map((u) => ({
...
function getChatPresenceStatus(chat: Chat): string {
    ...
    if (chat.type === "group" || chat.type === "team") {
        ...
        const hasOnline = chat.participants.some((p) => {
            ...
            return (
                realtime?.status === "online" || p.presence_status === "online"
            );
        });
...
<template>
    <div
        ref="panelRef"
        class="minichat-panel"
        :class="`chat-theme-${themeStore.chatTheme}`"
    >
        <!-- Header -->
        <div class="minichat-panel-header">
            ...
            <div class="minichat-panel-actions">
                ...
                <button
                    ...
                    :title="
                        miniChatStore.anchoringMode === 'docked'
                            ? 'Switch to Free Mode'
                            : 'Dock Windows'
                    "
                    @click="
                        miniChatStore.setAnchoringMode(
                            miniChatStore.anchoringMode === 'docked'
                                ? 'free'
                                : 'docked',
                        )
                ...
                <button class="minichat-panel-action" @click="emit('close')">
                    <Icon name="X" :size="16" />
                </button>
            </div>
        </div>
        ...
        <div class="minichat-panel-search">
            ...
            <input
                ...
                type="text"
                :placeholder="
                    miniChatStore.activeTab === 'invites'
                        ? 'Search invites...'
                        : 'Search chats...'
                "
            />
        ...
        <div class="minichat-panel-content">
            ...
            <template v-if="miniChatStore.activeTab === 'chats'">
                ...
                <div
                    v-else-if="filteredDmChats.length === 0"
                    class="minichat-empty"
                >
            ...
            <template v-else-if="miniChatStore.activeTab === 'groups'">
                ...
                <div
                    v-else-if="filteredGroupChats.length === 0"
                    class="minichat-empty"
                >
            ...
            <template v-else-if="miniChatStore.activeTab === 'invites'">
                <div
                    v-if="chatStore.invites.length === 0"
                    class="minichat-empty"
                >
                ...
                >
                    <Avatar
                        ...
                        :fallback="invite.inviter_name?.charAt(0) || '?'"
                        :color="invite.color"
                        size="md"
                    />
