...
const allSelected = computed({
    get: () =>
        ...
        tickets.value.every((t) => selectedTickets.value.has(t.id)),
    set: (value: boolean) => {
        if (value) {
...
const someSelected = computed(
    () =>
        tickets.value.some((t) => selectedTickets.value.has(t.id)) &&
        !allSelected.value,
);
...
function emitSelected() {
    const selected = tickets.value.filter((t) =>
        selectedTickets.value.has(t.id),
    );
...
<template>
    <Card
        padding="none"
        class="overflow-hidden shadow-sm border border-[var(--border-default)]"
    >
        <div
            class="px-6 py-4 border-b border-[var(--border-default)] bg-[var(--surface-elevated)]"
        >
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead
                    class="bg-gray-100 dark:bg-gray-800/50 border-b-2 border-[var(--border-default)]"
                >
                    <tr>
                ...
                <tbody
                    class="divide-y divide-[var(--border-muted)] bg-[var(--surface-elevated)]"
                >
                    <tr v-if="tickets.length === 0 && !loading">
                        ...
                        >
                            <div class="flex flex-col items-center gap-2">
                                <svg
                                    class="w-12 h-12 text-[var(--text-muted)] opacity-50"
                                    fill="none"
                                    stroke="currentColor"
                                    viewBox="0 0 24 24"
                                >
                    ...
                    >
                        ...
                        <td class="px-6 py-4 text-[var(--text-secondary)]">
                            {{ formatDate(ticket.created_at, "MMM d, yyyy") }}
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <!-- Footer / Pagination -->
        <div
            class="px-6 py-4 border-t border-[var(--border-default)] bg-[var(--surface-base)] flex items-center justify-between"
        >
