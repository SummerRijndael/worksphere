...
const canResend = computed(() => currentConfig.value?.canResend && countdown.value === 0);
...
async function verify() {
    const codeToVerify = useRecoveryCode.value ? recoveryCode.value : code.value;
    ...
    if (!useRecoveryCode.value) {
        if (currentConfig.value && currentConfig.value.requiresSend && !hasSentCode.value) {
...
<template>
    <div class="min-h-screen flex items-center justify-center bg-[var(--surface-primary)] p-4">
        <div class="w-full max-w-md space-y-8">
            ...
            <div v-if="!currentMethod && !useRecoveryCode && availableMethods.length > 0" class="space-y-4">
                <button
                ...
                >
                    <div class="p-2 rounded-lg bg-[var(--surface-secondary)] group-hover:bg-[var(--surface-primary)] transition-colors">
                        <component :is="methodConfig[method]?.icon" class="h-5 w-5 text-[var(--text-primary)]" />
                    </div>
                    <div class="flex-1">
                    ...
                    <ChevronRight class="h-4 w-4 text-[var(--text-muted)] group-hover:text-[var(--text-primary)]" />
                </button>
                ...
                <div class="relative py-4">
                     ...
                     <div class="relative flex justify-center text-xs">
                         <span class="bg-[var(--surface-primary)] px-2 text-[var(--text-muted)]">or</span>
                     </div>
                </div>
            ...
            <Card v-else class="p-6">
                <!-- Back Button -->
                <div class="flex items-center gap-2 mb-6" v-if="availableMethods.length > 1 && !useRecoveryCode">
                    <button 
                ...
                <div class="text-center mb-6" v-if="!useRecoveryCode">
                    <component :is="currentConfig?.icon" class="h-8 w-8 mx-auto text-[var(--interactive-primary)] mb-2" />
                    <h3 class="text-lg font-semibold text-[var(--text-primary)]">
                        {{ currentConfig?.name }}
                    </h3>
                    <div v-if="currentConfig?.requiresSend" class="text-sm text-[var(--text-secondary)]">
                        {{ hasSentCode ? 'Code sent to ' + maskedContact : 'We need to verify your identity.' }}
                    </div>
                ...
                <div v-if="useRecoveryCode" class="text-center mb-6">
                    <Key class="h-8 w-8 mx-auto text-[var(--interactive-primary)] mb-2" />
                     <h3 class="text-lg font-semibold text-[var(--text-primary)]">
                        Recovery Code
                    </h3>
                ...
                <div class="space-y-6">
                    ...
                    <div v-if="!useRecoveryCode && currentConfig?.requiresSend && !hasSentCode">
                        <p class="text-sm text-[var(--text-secondary)] text-center mb-4">
                            Click below to send a verification code to {{ maskedContact }}.
                        </p>
                        <Button 
                            full-width 
                            @click="sendCode" 
                    ...
                    <template v-else>
                        <div class="space-y-2">
                         ...
                         <!-- Resend Button -->
                        <div v-if="!useRecoveryCode && currentConfig?.canResend" class="text-center">
                            <button
                                :disabled="!canResend"
                                :class="[
                                ...
                                @click="sendCode"
                            >
                                <RefreshCw :class="['h-3 w-3', isSending && 'animate-spin']" />
                    ...
                    <div v-if="!useRecoveryCode" class="pt-2 text-center">
                     ...
                     <div v-else class="pt-2 text-center">
                         <button
                            class="text-xs text-[var(--text-muted)] hover:text-[var(--text-primary)]"
                            @click="useRecoveryCode = false; error = ''"
                        >
             ...
             <div v-if="(currentMethod || useRecoveryCode) && availableMethods.length > 0" class="text-center">
                 <button
                     @click="cancel"
