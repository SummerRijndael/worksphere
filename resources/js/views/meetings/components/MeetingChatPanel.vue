<template>
    <aside class="side-panel chat-panel">
        ...
        <div class="side-panel-body chat-messages" ref="messagesContainer">
            ...
            >
                <div v-if="shouldShowHeader(msg, index)" class="chat-message-header">
                    <span class="chat-sender-name">{{ getParticipantName(msg.participant_public_id) }}</span>
            ...
            <div v-if="meetingStore.chatMessages.length === 0" class="panel-empty">
...
function shouldShowHeader(msg: any, index: number) {
    ...
    if (prevMsg.participant_public_id !== msg.participant_public_id) return true;
...
async function submitMessage() {
    ...
    meetingStore.chatMessages.push({
        id: tempId,
        participant_public_id: meetingStore.localParticipant?.public_id || 'system',
        body: body,
    ...
    try {
        ...
        meetingStore.chatMessages = meetingStore.chatMessages.filter(m => m.id !== tempId);
    } finally {
...
function insertEmoji(emoji: string) {
    ...
    newMessage.value = currentValue.substring(0, start) + emoji + currentValue.substring(end);
