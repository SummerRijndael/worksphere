<template>
  <div ref="container" class="annotation-layer" 
    ...
    <Teleport to="body">
      <div 
        v-if="isLocal" 
        class="toolbox-overlay shadow-2xl border border-(--border-default) bg-(--surface-elevated)/95 backdrop-blur-xl rounded-2xl p-2 flex items-center gap-2 transition-all duration-300 ring-1 ring-black/5"
        :class="{ 'toolbox-overlay--collapsed': isCollapsed, 'toolbox-overlay--dragging': isDraggingToolbox }"
        :style="{ 
      ...
      >
        ...
        <template v-if="!isCollapsed">
          <!-- Tool Selection -->
          <div class="flex items-center bg-(--surface-subtle) rounded-xl p-1 gap-1">
            <button 
              @click="currentTool = 'brush'" 
              class="p-2 rounded-lg transition-all"
              :class="currentTool === 'brush' ? 'bg-(--brand-primary) text-white shadow-sm' : 'text-(--text-secondary) hover:bg-(--surface-hover)'"
              title="Pencil"
            >
            ...
            <button 
              ...
              class="p-2 rounded-lg transition-all"
              :class="currentTool === 'rect' ? 'bg-(--brand-primary) text-white shadow-sm' : 'text-(--text-secondary) hover:bg-(--surface-hover)'"
              title="Rectangle"
            >
            ...
            <button 
              ...
              class="p-2 rounded-lg transition-all"
              :class="currentTool === 'circle' ? 'bg-(--brand-primary) text-white shadow-sm' : 'text-(--text-secondary) hover:bg-(--surface-hover)'"
              title="Circle"
            >
            ...
            <button 
              ...
              class="p-2 rounded-lg transition-all"
              :class="currentTool === 'text' ? 'bg-(--brand-primary) text-white shadow-sm' : 'text-(--text-secondary) hover:bg-(--surface-hover)'"
              title="Text"
            >
              <Type class="w-4 h-4" />
            </button>
            <button 
              ...
              class="p-2 rounded-lg transition-all"
              :class="currentTool === 'eraser' ? 'bg-(--brand-primary) text-white shadow-sm' : 'text-(--text-secondary) hover:bg-(--surface-hover)'"
              title="Eraser"
            >
          ...
          <div class="flex items-center bg-(--surface-subtle) rounded-xl p-1 gap-1">
            <button 
              @click="undo" 
              class="p-2 rounded-lg text-(--text-secondary) hover:bg-(--surface-hover) disabled:opacity-30"
          ...
          <div class="flex items-center gap-1.5 px-1">
            <button 
              ...
              class="w-6 h-6 rounded-full border-2 transition-transform hover:scale-110 active:scale-95"
              :style="{ backgroundColor: color }"
              :class="currentColor === color ? 'border-(--brand-primary) ring-2 ring-(--brand-primary)/20' : 'border-transparent'"
            />
          </div>
          ...
          <div class="flex items-center gap-1">
            <button 
              ...
              class="p-2 rounded-lg transition-all text-(--text-secondary) hover:bg-(--surface-hover)"
              :class="{ 'text-(--brand-primary) bg-(--brand-primary)/10 font-bold': currentWidth === size }"
            >
              <div :style="{ width: size + 'px', height: size + 'px' }" class="bg-current rounded-full" />
            </button>
          </div>
        ...
        <button 
          ...
          class="p-2 rounded-lg text-(--text-secondary) hover:bg-(--surface-hover) transition-all"
          :title="isCollapsed ? 'Expand Toolbox' : 'Collapse Toolbox'"
        >
          <ChevronDown v-if="!isCollapsed" class="w-4 h-4" />
          <PenTool v-else class="w-4 h-4 text-(--brand-primary)" />
        </button>
      </div>
    </Teleport>
    ...
    <!-- Text Input Overlay -->
    <div v-if="isEditingText" class="absolute z-50 pointer-events-auto" :style="{ left: textPosition.x + 'px', top: textPosition.y + 'px' }">
      <input 
        ref="textRef"
        v-model="textInput" 
        class="bg-transparent border-b-2 border-(--brand-primary) outline-none text-white font-bold p-1 shadow-2xl"
        :style="{ color: currentColor, fontSize: (currentWidth * 4) + 'px' }"
        @blur="finishText"
...
import { Pencil, Eraser, Trash2, Square, Circle, Type, Undo2, Redo2, GripVertical, ChevronDown, PenTool } from 'lucide-vue-next';
...
// Toolbox State
const currentTool = ref<'brush' | 'eraser' | 'rect' | 'circle' | 'text'>('brush');
const currentColor = ref('#ff0000');
const currentWidth = ref(4);
const colors = ['#ffffff', '#000000', '#ff0000', '#00ff00', '#0000ff', '#f59e0b', '#8b5cf6'];
...
watch(() => props.initialLines, (newElements) => {
...
const handleMouseDown = (e: any) => {
  ...
  if (currentTool.value === 'brush' || currentTool.value === 'eraser') {
    elements.value.push({
      ...
      type: 'line',
      points: [pos.x, pos.y],
      normalizedPoints: [pos.x / stageConfig.width, pos.y / stageConfig.height],
      lineCap: 'round',
      lineJoin: 'round',
    });
