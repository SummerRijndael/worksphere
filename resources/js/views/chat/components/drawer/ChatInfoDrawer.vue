...
const isOwner = computed(() => {
    ...
    const self = props.chat.participants.find(
...
const confirmKick = async () => {
    ...
    try {
        await chatStore.kickMember(
            props.chat.public_id,
            kickTarget.value.public_id,
        );
    } catch (e) {
        console.error(e);
    } finally {
...
async function handleRenameGroup() {
    if (!props.chat || !newGroupName.value.trim() || renamingGroup.value)
        return;

    renamingGroup.value = true;
    try {
        await chatStore.updateGroup(props.chat.public_id, {
    ...
    <DialogRoot :open="open" @update:open="handleOpenChange">
        <DialogPortal>
            ...
            >
                <!-- Header -->
                <div
                    class="flex-none flex items-center justify-between p-4 border-b border-[var(--border-default)] bg-[var(--surface-elevated)]"
                >
                    <DialogTitle
                ...
                <div v-if="chat" class="flex-1 overflow-y-auto">
                    ...
                    <AccordionRoot type="multiple" class="w-full">
                        ...
                        >
                            <AccordionTrigger
                                class="flex items-center justify-between w-full p-4 text-sm font-semibold text-[var(--text-primary)] hover:bg-[var(--surface-secondary)] transition-colors"
                            >
                            ...
                            <AccordionContent
                                class="bg-[var(--surface-secondary)]/30 animate-accordion-down overflow-hidden"
                            >
                        ...
                        >
                            ...
                            >
                                <div class="p-2">
                                    <div
                                        class="flex items-center justify-between px-2 py-2 mb-2"
                                    >
                                        <span
                                            class="text-xs font-medium text-[var(--text-tertiary)]"
                                            >{{
                                                ...
                                                >
                                                    ...
                                                    <span
                                                        v-if="
                                                            ...
                                                            'owner'
                                                        "
                                                        class="text-[10px] bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 px-1.5 py-0.5 rounded"
                                                        >Owner</span
                                                    >
                                                </div>
                                            </div>
                            ...
                            >
                                <div class="p-4">
                                    ...
                                    <template v-else-if="mediaItems.length">
                                        <div
                                            class="mv-gallery grid grid-cols-3 gap-2"
                                        >
                                            <div
                                            ...
                                            >
                                                ...
                                                <div
                                                    v-else
                                                    class="flex items-center justify-center h-full text-[var(--text-tertiary)]"
                                                >
                                                    <Icon
                                                        name="FileText"
                                                        :size="24"
                                        ...
                                        >
                                            <div
                                                class="w-5 h-5 border-2 border-[var(--interactive-primary)] border-t-transparent rounded-full animate-spin opacity-50"
                                            />
                                        </div>
                                    </template>
                                    ...
                                    <div
                                        v-else
                                        class="text-center py-8 text-sm text-[var(--text-tertiary)]"
                                    >
                ...
                <div
                    v-if="chat && isGroup"
                    class="flex-none p-4 bg-[var(--surface-tertiary)] border-t border-[var(--border-default)]"
                >
                    <button
                        v-if="!isOwner"
