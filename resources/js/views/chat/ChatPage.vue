...
const activeHeader = computed(() => {
    ...
    if (activeChat.value.type === "dm") {
        const other = activeChat.value.participants.find(
...
function handleStartCall(callType: "video" | "audio") {
    ...
    const other =
        activeChat.value.type === "dm"
            ? activeChat.value.participants.find(
                  (p) => p.public_id !== currentUserPublicId.value,
              )
            : {
                  public_id: "group",
                  name: activeChat.value.name || "Group Chat",
                  avatar: null,
              };
        ...
        >
            <template v-if="!authStore.isImpersonating">
                ...
                >
                    ...
                    >
                        <div
                            class="w-2 h-2 bg-(--text-tertiary) rounded-full animate-bounce [animation-delay:-0.3s]"
                        ></div>
                        <div
                            class="w-2 h-2 bg-(--text-tertiary) rounded-full animate-bounce [animation-delay:-0.15s]"
                        ></div>
                        <div
                            class="w-2 h-2 bg-(--text-tertiary) rounded-full animate-bounce"
                        ></div>
                    </div>
                    <span class="font-medium text-xs animate-pulse">{{
                        typingIndicator
                    }}</span>
                </div>
            ...
            <LockedFeature
                ...
                description="To protect user privacy, chat access is disabled during impersonation. You cannot view messages or participate in conversations."
            />
        </main>
