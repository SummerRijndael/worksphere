...
const presenceStatus = computed(() => {
    ...
    const realTimeUser = presenceUsers.value.get(
        user.value.public_id || userId,
    );
...
const saveUser = async (isConfirmed = false) => {
    ...
    const roleChanged =
        canManageRoles() &&
        editForm.value.role !== (user.value.roles?.[0]?.name || null);
    const statusChanged =
        canManageStatus() && editForm.value.status !== user.value.status;
...
const revokeSessions = async (sessionIds = []) => {
    ...
    try {
        ...
        toast.success(
            isAll ? "All sessions revoked" : "Selected sessions revoked",
        );
    ...
    <div class="flex flex-col h-full w-full p-6 space-y-6">
        ...
        <div>
            <Button
                variant="ghost"
                size="sm"
                class="gap-2 text-[var(--text-secondary)] hover:text-[var(--text-primary)] pl-0 hover:bg-transparent"
        ...
        <div v-if="isLoading" class="space-y-6">
            <div
                class="h-32 bg-[var(--surface-primary)] rounded-xl border border-[var(--border-muted)] animate-pulse"
            ></div>
            <div
                class="h-10 w-64 bg-[var(--surface-primary)] rounded-lg animate-pulse"
            ></div>
            <div
                class="h-64 bg-[var(--surface-primary)] rounded-xl border border-[var(--border-muted)] animate-pulse"
            ></div>
        </div>
        ...
        <div v-else class="space-y-6">
            ...
            >
                ...
                >
                    ...
                    <div class="relative group shrink-0">
                        ...
                        />
                        <button
                            v-if="user.avatar_url && canEdit()"
                            @click="removeAvatar"
                            class="absolute -top-1 -right-1 p-1 rounded-full bg-white/90 dark:bg-gray-800/90 text-red-600 border border-gray-200 dark:border-gray-700 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-50 dark:hover:bg-red-900/20 shadow-sm"
                            title="Remove Avatar"
                    ...
                    <div class="flex gap-2">
                        ...
                        <template v-else>
                            <Button
                                ...
                                class="bg-yellow-500/10 text-yellow-600 border-yellow-200 hover:bg-yellow-500/20 dark:border-yellow-800 dark:text-yellow-400"
                            >
            ...
            <!-- Tabs Navigation -->
            <div
                class="flex items-center gap-1 border-b border-[var(--border-default)] overflow-x-auto"
            >
                <button
                    v-for="tab in tabs"
                    :key="tab.id"
            ...
            <div class="min-h-[400px]">
                ...
                <div v-if="activeTab === 'profile'" class="w-full space-y-6">
                    ...
                    >
                        ...
                        <div class="p-6 divide-y divide-[var(--border-muted)]">
                            <div
                                class="grid grid-cols-1 md:grid-cols-3 gap-4 py-3 first:pt-0"
                            >
                                <dt
                                ...
                                <dd
                                    class="text-sm text-[var(--text-primary)] md:col-span-2"
                                >
                                    <input
                                        v-if="isEditing"
                            ...
                            <div
                                class="grid grid-cols-1 md:grid-cols-3 gap-4 py-3"
                            >
                                <dt
                                    class="text-sm font-medium text-[var(--text-secondary)]"
                                >
                            ...
                            >
                                ...
                                <dd
                                    class="text-sm text-[var(--text-primary)] md:col-span-2 flex items-center gap-2"
                                >
                                    <template v-if="isEditing">
                                        <div
                                            class="flex items-center gap-2 px-3 py-2 bg-[var(--surface-secondary)] rounded-lg border border-[var(--border-default)] max-w-sm"
                                        >
                                            <Lock
                                        ...
                                        <span
                                            ...
                                            title="Email can only be changed by the user"
                                            >Self-service only</span
                                        >
                                    </template>
                                    <template v-else>
                                        <span>{{
                            ...
                            >
                                ...
                                <dd
                                    class="text-sm md:col-span-2 flex items-center justify-between gap-2"
                                >
                            ...
                            <div
                                class="grid grid-cols-1 md:grid-cols-3 gap-4 py-3 border-none pb-0"
                            >
                                <dt
                                    class="text-sm font-medium text-[var(--text-secondary)]"
                                >
                ...
                <div v-if="activeTab === 'security'" class="w-full space-y-6">
                    ...
                    >
                        <div
                            class="p-4 border-b border-[var(--border-muted)] flex items-center gap-2"
                        >
                            <Shield
                        ...
                        <div class="p-6 divide-y divide-[var(--border-muted)]">
                            <!-- Password Info -->
                            <div
                                class="flex items-start justify-between py-4 first:pt-0"
                            >
                                <div class="space-y-1">
                                    <h4
                                    ...
                                    <div class="flex flex-col gap-1">
                                        ...
                                        <p
                                            v-if="user.password_last_updated_at"
                                            class="text-xs text-[var(--text-muted)]"
                                        >
                                            Last changed:
                                            {{
                                    ...
                                    <div class="flex flex-col gap-2 mt-2">
                                        ...
                                        >
                                            <div
                                                ...
                                                :key="account.id"
                                                class="flex items-center gap-3 p-2 rounded-lg border border-[var(--border-default)] bg-[var(--surface-secondary)]/50"
                                            >
                                                <component
                                                    :is="
                                                        account.provider ===
                            ...
                            <div class="flex flex-col py-4 gap-4">
                                <div class="flex items-center justify-between">
                                    <div class="space-y-1">
                                        ...
                                        >
                                            Manage devices where this user is
                                            currently logged in.
                                        </p>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <Button
                                            ...
                                            size="sm"
                                            @click="
                                                revokeSessions(selectedSessions)
                                            "
                                ...
                                >
                                    <table class="w-full text-sm text-left">
                                        <thead
                                            class="bg-[var(--surface-secondary)] text-[var(--text-secondary)] font-medium border-b border-[var(--border-default)]"
                                        >
                                            <tr>
                                        ...
                                        >
                                            ...
                                            <tr
                                                ...
                                                :key="session.id"
                                                class="group hover:bg-[var(--surface-secondary)]/30 transition-colors"
                                            >
                                                <td class="px-4 py-3">
                                                    <input
                                                        type="checkbox"
                                                ...
                                                <td class="px-4 py-3">
                                                    ...
                                                    >
                                                        <div
                                                            class="w-8 h-8 rounded-lg bg-[var(--surface-secondary)] flex items-center justify-center text-[var(--text-muted)]"
                                                        >
                                                            <component
                                                                :is="
                                                                    session
                                                ...
                                                <td
                                                    class="px-4 py-3 font-mono text-xs text-[var(--text-secondary)]"
                                                >
                                                    <div>
                                                        {{ session.ip_address }}
                                                    </div>
                                                    <div
                                                        ...
                                                        :title="`${session.location.city}, ${session.location.state}, ${session.location.country}`"
                                                    >
                                                        {{
                                                            session.location
                                                                .city
                                                ...
                                                <td
                                                    class="px-4 py-3 text-right text-[var(--text-secondary)]"
                                                >
                            ...
                            <!-- MFA / 2FA Enforcement -->
                            <div
                                class="flex flex-col py-4 border-none pb-0 gap-4"
                            >
                                <div class="space-y-1">
                                    <h4
                                        class="text-sm font-medium text-[var(--text-primary)] flex items-center gap-2"
                                    ...
                                    <p
                                        class="text-sm text-[var(--text-secondary)] text-balance"
                                    >
                                        Manage 2FA enforcement for this user
                                        account.
                                    </p>
                                </div>
                ...
                <div v-if="activeTab === 'access'" class="space-y-6">
                    ...
                    <div
                        v-if="canManageStatus() || canManageRoles()"
                        class="bg-[var(--surface-primary)] rounded-xl border border-[var(--border-muted)] overflow-hidden"
                    >
                        <div class="p-4 border-b border-[var(--border-muted)]">
                            <h3 class="font-medium text-[var(--text-primary)]">
                            ...
                            <p class="text-xs text-[var(--text-muted)] mt-1">
                                Manage user status and role assignments with
                                audit trail
                            </p>
                        </div>
                        <div class="p-6">
                        ...
                        <div class="p-6">
                            <div class="flex flex-wrap gap-3">
                                <div
                                    ...
                                    :key="role.name"
                                    class="flex items-center gap-2 px-3 py-2 bg-[var(--surface-secondary)] rounded-lg border border-[var(--border-default)]"
                                >
                                    <div
                        ...
                        <div class="p-6">
                            <div class="flex flex-wrap gap-2">
                                <span
                                    ...
                                    class="px-2.5 py-1 bg-[var(--surface-secondary)] text-[var(--text-secondary)] rounded-md text-xs border border-[var(--border-default)]"
                                >
                    ...
                    >
                        <div
                            class="p-4 border-b border-[var(--border-muted)] flex items-center justify-between"
                        >
                            <div>
                                <h3
                                    class="font-medium text-[var(--text-primary)]"
                                ...
                                >
                                    Grant or block specific permissions for this
                                    user
                                </p>
                            </div>
                            <Button
                ...
                <div v-if="activeTab === 'activity'" class="space-y-6">
                    <div
                        class="bg-[var(--surface-primary)] rounded-xl border border-[var(--border-muted)] overflow-hidden flex flex-col"
                        style="max-height: calc(100vh - 20rem)"
                    >
                        <!-- Header with Per Page Selector -->
                        <div
                            class="p-4 border-b border-[var(--border-muted)] flex items-center justify-between shrink-0"
                        >
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-8 h-8 rounded-lg bg-[var(--interactive-primary)]/10 flex items-center justify-center"
                                >
                                    <History
                        ...
                        <div
                            v-if="isLogsLoading"
                            class="p-8 flex flex-col items-center justify-center gap-3 flex-1"
                        >
                            <Loader2
                        ...
                        >
                            <div
                                class="w-12 h-12 rounded-full bg-[var(--surface-secondary)] flex items-center justify-center"
                            >
                                <FileText
                            ...
                            <div class="text-center">
                                ...
                                <p class="text-xs text-[var(--text-muted)]">
                                    User actions will appear here once they
                        ...
                        <div
                            v-else
                            class="divide-y divide-[var(--border-default)] overflow-y-auto flex-1"
                        >
                            <div
                            ...
                            >
                                <!-- Action Icon -->
                                <div
                                    class="w-10 h-10 rounded-xl bg-[var(--surface-secondary)] flex items-center justify-center text-[var(--text-muted)] group-hover:bg-[var(--surface-tertiary)] transition-colors shrink-0"
                                >
                                    <component
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div
                                        class="flex items-center gap-3 text-xs text-[var(--text-muted)]"
                                    >
                                        <span
                                            v-if="
                                                log.ip_address ||
                                ...
                                <!-- View Button -->
                                <button
                                    class="p-2 rounded-lg text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--surface-tertiary)] transition-colors opacity-0 group-hover:opacity-100"
                                >
                                    <Eye class="w-4 h-4" />
                                </button>
                            </div>
                        ...
                        <div
                            ...
                            class="px-4 py-3 border-t border-[var(--border-default)] bg-[var(--surface-secondary)]/30 flex items-center justify-between shrink-0"
                        >
                            <span class="text-xs text-[var(--text-muted)]">
                                Showing
                                <span
                                    class="font-medium text-[var(--text-primary)]"
                                    >{{ logsPagination.from || 1 }}</span
                                >
                                to
                                <span
                                    class="font-medium text-[var(--text-primary)]"
                                    >{{
                                        logsPagination.to || auditLogs.length
                                    }}</span
                                >
                                of
                                <span
                                    class="font-medium text-[var(--text-primary)]"
                                    >{{
                                        logsPagination.total || auditLogs.length
                                    }}</span
                                >
                                activities
                            </span>
                            <div class="flex items-center gap-2">
                                ...
                                <span
                                    class="text-xs text-[var(--text-secondary)] px-2"
                                >
                                    Page {{ logsPagination.current_page }} of
                ...
                >
                    <div v-if="selectedLog" class="space-y-6">
                        <!-- Header -->
                        <div
                            class="flex items-start gap-4 p-4 bg-[var(--surface-secondary)] rounded-lg"
                        >
                            <div
                                class="w-12 h-12 rounded-xl bg-[var(--surface-tertiary)] flex items-center justify-center"
                            >
                                <component
                                    :is="getActionIcon(selectedLog.action)"
                                    class="w-6 h-6 text-[var(--text-muted)]"
                                />
                            </div>
                            <div class="flex-1">
                                ...
                                <span
                                    ...
                                    :class="[
                                        severityColors[selectedLog.severity] ||
                                            severityColors.info,
                                        'inline-flex px-2.5 py-1 text-xs font-medium rounded border capitalize',
                                    ]"
                                >
                        ...
                        <!-- Metadata -->
                        <div
                            v-if="selectedLog.metadata || selectedLog.context"
                            class="space-y-3"
                        >
                            <h4
                                class="text-sm font-medium text-[var(--text-primary)]"
                            ...
                            <pre
                                class="text-xs bg-[var(--surface-secondary)] p-3 rounded-lg overflow-auto max-h-48 text-[var(--text-secondary)]"
                                >{{
                                    JSON.stringify(
                ...
                <Modal
                    v-model:open="showConfirmUpdateModal"
                    title="Security Confirmation"
                    size="md"
                >
                    <div class="space-y-4">
                        <div
                            class="p-3 bg-amber-500/10 border border-amber-200 dark:border-amber-800 rounded-lg flex gap-3"
                        >
                            <ShieldAlert
                            ...
                            >
                                You are performing a critical action (changing
                                user role or status). Please confirm your
                                identity and provide a reason for this change.
                            </div>
                        </div>
                        ...
                        <div
                            class="flex justify-end gap-3 pt-4 border-t border-[var(--border-default)]"
                        >
                            <Button
                                variant="ghost"
