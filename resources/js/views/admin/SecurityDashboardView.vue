...
import SecurityIpDetailsModal from "@/components/admin/SecurityIpDetailsModal.vue";
...
onMounted(() => {
    ...
    const timer = setInterval(() => {
        analytics.value.active_blocks = analytics.value.active_blocks.map(
            (block) => {
                if (block.remaining_seconds > 0) {
                    return {
                        ...block,
                        remaining_seconds: block.remaining_seconds - 60,
                    };
                }
                return block;
            },
...
<template>
    <div class="p-4 sm:p-6 lg:p-8 max-w-[1600px] mx-auto space-y-8">
        <!-- Header -->
        <div
            class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-(--surface-card) p-6 rounded-2xl shadow-sm border border-(--border-default)"
        >
            <div>
                <h1
                ...
                <p class="text-(--text-secondary) mt-1 text-lg">
                    Real-time monitoring and management of system security
                    protocols.
                </p>
            </div>
            <div class="flex flex-wrap items-center gap-3">
                <!-- Period Selector -->
                <div
                    class="flex items-center bg-(--surface-secondary) rounded-lg p-1 border border-(--border-default)"
                >
                ...
                >
                    <ArrowPathIcon
                        class="w-5 h-5"
                        :class="{
                            'animate-spin': loadingStats || loadingCharts,
                        }"
                    />
                    Refresh Data
                </button>
        ...
        <!-- Navigation Tabs -->
        <div
            class="flex border-b border-(--border-default) overflow-x-auto gap-8 no-scrollbar bg-(--surface-card) px-6 rounded-xl shadow-sm"
        >
            <button
        ...
        <div class="mt-8 transition-all duration-300">
            ...
            >
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <!-- Top IP Offenders -->
                    <div
                        class="bg-(--surface-card) p-6 rounded-2xl shadow-sm border border-(--border-default)"
                    >
                        <h3
                            class="text-lg font-bold mb-6 text-(--text-primary) flex items-center gap-2"
                        >
                        ...
                        <div class="overflow-hidden">
                            <table class="w-full text-left">
                                <thead
                                    class="text-xs font-bold text-(--text-tertiary) uppercase tracking-wider"
                                >
                                    <tr>
                                        <th class="pb-3 px-2">IP Address</th>
                                        <th class="pb-3 px-2">Location</th>
                                        <th class="pb-3 px-2 text-right">
                                            Attempts
                                        </th>
                                    </tr>
                                </thead>
                                ...
                                >
                                    <tr
                                        v-for="offender in analytics
                                            .top_offenders.ips"
                                        :key="offender.ip_address"
                                        class="hover:bg-(--surface-subtle) transition-colors"
                                    >
                                        <td
                                            class="py-3 px-2 font-mono text-sm text-(--text-primary)"
                                        >
                                            {{ offender.ip_address }}
                                        </td>
                                        <td
                                            class="py-3 px-2 text-sm text-(--text-secondary)"
                                        >
                                            <div
                                                class="flex items-center gap-2"
                                            >
                                                <span
                                                    v-if="offender.country_code"
                                                    class="text-lg"
                                                    >{{
                                                        offender.country_code
                                                            .toUpperCase()
                                                            .replace(
                                                                /./g,
                                                                (char) =>
                                                ...
                                                <span>{{
                                                    offender.city ||
                                                    offender.country_name ||
                                                    "Unknown"
                                                }}</span>
                                            </div>
                                        </td>
                                        <td
                                            class="py-3 px-2 text-sm font-bold text-red-500 text-right"
                                        >
                                            {{ offender.total_attempts }}
                                        </td>
                                    </tr>
                                    <tr
                                        v-if="
                                            analytics.top_offenders.ips
                                                .length === 0
                                        "
                                    >
                                        <td
                                            colspan="3"
                                            class="py-8 text-center text-(--text-tertiary) italic"
                                        >
                                            No data available
                                        </td>
                                    </tr>
                    ...
                    >
                        ...
                        <div class="overflow-hidden">
                            <table class="w-full text-left">
                                ...
                                >
                                    <tr
                                        v-for="user in analytics.top_offenders
                                            .users"
                                        :key="user.user_id"
                                        class="hover:bg-(--surface-subtle) transition-colors"
                                    >
                                        <td
                                            class="py-3 px-2 text-sm font-bold text-(--text-primary)"
                                        >
                                            {{ user.user_name }}
                                        </td>
                                        <td
                                        ...
                                        <td
                                            class="py-3 px-2 text-sm font-bold text-orange-500 text-right"
                                        >
                                            {{ user.count }}
                                        </td>
                                    </tr>
                ...
                <div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
                    ...
                    <div class="xl:col-span-1">
                        <div
                            class="bg-(--surface-card) p-6 rounded-2xl shadow-sm border border-(--border-default) h-full flex flex-col min-h-[600px] max-h-[900px]"
                        >
                            <div
                            ...
                            >
                                <SecurityActivityFeed
                                    :activities="analytics.security_logs"
                                    :loading="false"
                                />
                            </div>
                        </div>
            ...
            <!-- Threat Map Tab -->
            <div
                v-if="activeTab === 'threat-map'"
                class="animate-in fade-in slide-in-from-bottom-4 duration-500"
            >
                <div
                    class="bg-(--surface-card) p-8 rounded-2xl shadow-sm border border-(--border-default)"
                >
                    <div class="mb-8">
                        ...
                        <p class="text-(--text-secondary) mt-2">
                            Visualizing the origin and intensity of detected
                            security threats across the globe.
                        </p>
                    </div>
                    <SecurityMap
                        :data="mapData"
            ...
            <div
                v-if="activeTab === 'suspicious-activity'"
                class="animate-in fade-in slide-in-from-bottom-4 duration-500"
            >
                <div
                    class="bg-(--surface-card) p-8 rounded-2xl shadow-sm border border-(--border-default)"
                >
                    <div class="mb-8">
                        ...
                        <p class="text-(--text-secondary) mt-2">
                            Comprehensive logs of all system-access attempts,
                            profile changes, and security events.
                        </p>
                    </div>
                    <SuspiciousActivityTable @investigate="investigateIp" />
                </div>
            </div>

            <!-- Active Blocks Tab -->
            <div
                v-if="activeTab === 'active-blocks'"
                class="animate-in fade-in slide-in-from-bottom-4 duration-500"
            >
                <div
                    class="bg-(--surface-card) p-8 rounded-2xl shadow-sm border border-(--border-default)"
                >
                    <div class="mb-8 flex justify-between items-center">
                        <div>
                            ...
                            <p class="text-(--text-secondary) mt-2">
                                Currently enforced network-level blocks with
                    ...
                    <div
                        class="overflow-hidden bg-(--surface-subtle) rounded-xl border border-(--border-subtle)"
                    >
                        <table class="w-full text-left">
                            <thead
                                class="text-xs font-bold text-(--text-tertiary) uppercase tracking-wider bg-(--surface-card)"
                            >
                                <tr>
                            ...
                            <tbody class="divide-y divide-(--border-subtle)">
                                <tr
                                    v-for="block in analytics.active_blocks"
                                    :key="block.id"
                                    class="hover:bg-(--surface-card) transition-colors"
                                >
                                    <td
                                        class="py-4 px-6 font-mono font-bold text-(--text-primary)"
                                    >
                                        {{ block.ip }}
                                    </td>
                                    ...
                                    <td class="py-4 px-6 text-center">
                                        ...
                                        >
                                            ...
                                            {{
                                                formatTimer(
                                                    block.remaining_seconds,
                                                )
                                            }}
                                        </span>
                                    </td>
                                    <td
                                        class="py-4 px-6 text-sm text-(--text-tertiary)"
                                    >
                                        {{
                                            new Date(
                                                block.created_at,
                                ...
                                <tr v-if="analytics.active_blocks.length === 0">
                                    <td
                                        colspan="4"
                                        class="py-12 text-center text-(--text-tertiary)"
                                    >
                                        <ShieldCheckIcon
                                            class="w-12 h-12 mx-auto mb-3 opacity-20"
                                        />
                                        <p class="font-medium text-lg">
                                            No active IP blocks detected.
                                        </p>
                                        <p class="text-sm">
                                            The system is currently clear of
                                            enforced network bans.
                                        </p>
                                    </td>
                                </tr>
                            </tbody>
            ...
            <!-- Whitelisted IPs Tab -->
            <div
                v-if="activeTab === 'whitelisted-ips'"
                class="animate-in fade-in duration-300"
            >
        ...
        <!-- Investigation Modal -->
        <SecurityIpDetailsModal
            :show="showIpDetails"
            :ip="selectedIp"
            @close="showIpDetails = false"
        />
    </div>
</template>
