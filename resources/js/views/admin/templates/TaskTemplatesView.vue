...
const fetchTemplates = async () => {
    ...
    try {
        if (teamId) {
            const data = await taskTemplateService.getAll(teamId);
...
const handleDelete = async (id: string) => {
    ...
    try {
        await taskTemplateService.delete(teamId, id);
...
<template>
    <div class="space-y-6">
        ...
        <Card>
            ...
            <div v-else-if="templates?.length === 0" class="p-12 text-center flex flex-col items-center justify-center">
                ...
                <p class="text-[var(--text-secondary)] mt-1 mb-6 max-w-sm">
                    Create templates to quickly generate tasks with predefined checklists and settings.
                </p>
                <Button @click="handleCreate">
                    <Plus class="h-4 w-4 mr-2" />
                    Create Template
                ...
                <table class="w-full">
                    <thead class="bg-[var(--surface-secondary)] border-b border-[var(--border-default)]">
                        <tr>
                            ...
                            <th class="text-left px-6 py-3 text-xs font-medium text-[var(--text-secondary)] uppercase tracking-wider">Priority</th>
                    ...
                    <tbody class="divide-y divide-[var(--border-default)]">
                        <tr v-for="template in templates" :key="template.id" class="hover:bg-[var(--surface-secondary)]/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-sm text-[var(--text-secondary)]">
                                {{ template.default_priority ? template.default_priority.charAt(0).toUpperCase() + template.default_priority.slice(1) : '-' }}
                            </td>
                            <td class="px-6 py-4">
        ...
        <Modal 
            ...
            :title="selectedTemplate ? 'Edit Task Template' : 'Create Task Template'"
            size="2xl"
        >
            <TaskTemplateBuilder 
                v-if="isModalOpen"
