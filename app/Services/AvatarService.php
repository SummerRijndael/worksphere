...
class AvatarService implements AvatarContract
{
    ...
    protected function resolveUser(User $user, string $variant): AvatarData
    {
        ...
        if ($user->hasMedia('avatars')) {
            ...
            // If disk is not local (e.g., r2, s3, or overriding private), assume it exists.
            // file_exists() does not work gracefully with remote flysystem URLs.
            if ($media->disk !== 'local' && config("filesystems.disks.{$media->disk}.driver") !== 'local') {
